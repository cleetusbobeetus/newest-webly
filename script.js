...
window.addEventListener('scroll', () => {
    ...
    // PayPal Integration and Payment Method Handling
        ...
        if (window.paypal && document.getElementById('paypal-button-container')) {
            ...
            paypal.Buttons({
                createOrder: function(data, actions) {
                    // Get the selected service to determine amount
                    const serviceSelect = document.getElementById('service');
                    let amount = '10.00'; // Default amount
                    
                    // Set amount based on service selection
                    if (serviceSelect.value === 'starter') {
                        amount = '300.00';
                    } else if (serviceSelect.value === 'professional') {
                        amount = '7999.00';
                    } else if (serviceSelect.value === 'enterprise') {
                        amount = '19999.00';
                    } else if (serviceSelect.value === 'maintenance') {
                        amount = '99.00';
                    }
                    ...
                    return actions.order.create({
                        purchase_units: [{
                            ...
                            description: `Webly Industries - ${serviceSelect.options[serviceSelect.selectedIndex].text}`
                        }]
                    });
                },
                onApprove: function(data, actions) {
                    return actions.order.capture().then(function(details) {
                        ...
                        showNotification('Payment successful! Your message has been sent.', 'success');
                ...
                onError: function(err) {
                    ...
                    showNotification('PayPal payment failed. Please try again or use another payment method.', 'error');
                },
                onCancel: function(data) {
                    console.log('PayPal payment cancelled:', data);
                    showNotification('Payment cancelled. You can still send your message without payment.', 'info');
                }
